  ...
  return (
    <div className={`flex gap-2 ${className}`}>
      ...
      {measurements?.map((measurement) => {
        ...
        if (measurement.max === null || measurement.min === null)
          return (
            <DataUnavailableCard key={measurement.date} title={cardTitle} />
          );

        return (
          <Card key={measurement.date} title={cardTitle}>
            <div>
              <span className="font-semibold">Max:</span> {measurement.max}{" "}
              {TEMPERATURE_SYMBOLS[unit]}
            </div>
            <div>
              <span className="font-semibold">Min:</span> {measurement.min}{" "}
              {TEMPERATURE_SYMBOLS[unit]}
            </div>
          </Card>
        );
